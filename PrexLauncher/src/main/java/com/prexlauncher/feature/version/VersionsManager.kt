...
import com.prexlauncher.event.single.RefreshVersionsEvent.MODE.END
import com.prexlauncher.event.single.RefreshVersionsEvent.MODE.START
...
import com.prexlauncher.feature.version.utils.VersionInfoUtils
...
import com.prexlauncher.utils.stringutils.SortStrings
...
object VersionsManager {
     ...
     * @see com.prexlauncher.event.single.RefreshVersionsEvent
     */
    ...
    private fun processVersionFile(versionsHome: String, versionFile: File, refreshVersionInfo: Boolean) {
        if (versionFile.exists() && versionFile.isDirectory) {
            ...
            if (jsonFile.exists() && jsonFile.isFile) {
                isVersion = true
                val versionInfoFile = File(getPrexVersionPath(versionFile), "VersionInfo.json")
    ...
    /**
     * @return 获取 Prex 启动器版本标识文件夹
     */
    fun getPrexVersionPath(version: Version) = File(version.getVersionPath(), InfoDistributor.LAUNCHER_NAME)

    /**
     * @return 通过目录获取 Prex 启动器版本标识文件夹
     */
    fun getPrexVersionPath(folder: File) = File(folder, InfoDistributor.LAUNCHER_NAME)

    /**
     * @return 通过名称获取 Prex 启动器版本标识文件夹
     */
    fun getPrexVersionPath(name: String) = File(getVersionPath(name), InfoDistributor.LAUNCHER_NAME)

    /**
     * @return 获取当前版本设置的图标
     */
    fun getVersionIconFile(version: Version) = File(getPrexVersionPath(version), "VersionIcon.png")

    /**
     * @return 通过名称获取当前版本设置的图标
     */
    fun getVersionIconFile(name: String) = File(getPrexVersionPath(name), "VersionIcon.png")
