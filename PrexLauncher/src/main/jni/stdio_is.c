...
Java_com_prexlauncher_core_Logger_begin(JNIEnv *env, __attribute((unused)) jclass clazz, jstring logPath) {
    if (latestlog_fd != -1)
    {
    ...
    {
        jclass eventLogListener = (*env)->FindClass(env, "com/prexlauncher/core/Logger$eventLogListener");
...
JNIEXPORT void JNICALL Java_com_prexlauncher_core_Logger_appendToLog(JNIEnv *env, __attribute((unused)) jclass clazz, jstring text) {
...
JNIEXPORT void JNICALL
Java_com_prexlauncher_core_Logger_setLogListener(JNIEnv *env, __attribute((unused)) jclass clazz, jobject log_listener) {
...
JNIEXPORT void JNICALL
Java_com_prexlauncher_core_utils_JREUtils_setupExitMethod(JNIEnv *env, jclass clazz,
                                                        jobject context) {
    exitTrap_ctx = (*env)->NewGlobalRef(env,context);
    (*env)->GetJavaVM(env,&exitTrap_jvm);
    exitTrap_exitClass = (*env)->NewGlobalRef(env,(*env)->FindClass(env,"com/prexlauncher/ui/activity/ErrorActivity"));
